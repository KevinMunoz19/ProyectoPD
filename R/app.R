...
#print(my_db %>% tbl("recovered") %>% head(5))
#print(my_db %>% tbl("deaths") %>% head(5))
#print(my_db %>% tbl("conf") %>% head(5))

tablecon <- my_db %>% tbl("conf")
tablecon <- as.tibble(tablecon)
tablede <- my_db %>% tbl("deaths")
tablede <- as.tibble(tablede)
tablere <- my_db %>% tbl("recovered")
tablere <- as.tibble(tablere)


tablecon <- tablecon %>% select(-c("id"))
tablede <- tablede %>% select(-c("id"))
tablere <- tablere %>% select(-c("id"))


tablemerged <- merge(x=tablecon,y=tablede,by=c("date_d","state","country",'lat','lon'), all=TRUE)
tablemerged <- tablemerged %>% 
  rename(
    confirmed = value.x,
    deaths = value.y
  )

tablemergednew <- merge(x=tablemerged,y=tablere,by=c("date_d","state","country",'lat','lon'), all=TRUE)
tablemergednew <- tablemergednew %>% 
  rename(
    Lat = lat,
    Long = lon,
    PAIS = country,
...
tablecontemp <- tablemergednew %>% group_by(ESTADO, PAIS, Lat, Long) %>% summarise(confirmed = max(confirmed, na.rm = T))
tablecontemp <- data.frame(tablecontemp[,c("Lat","Long", "PAIS" ,"confirmed")])
temp <- subset(tablecontemp, !c(is.na(Long) | Long == 0))



#consolidado <- read.csv("consolidado.csv")
#print(consolidado)
consolidado <- tablemergednew   
...
server <- function(input, output, session) {
  ...
  output$mapa_O1 <- renderLeaflet({
    ...
    proj4string(table) = CRS("+init=epsg:4326")
